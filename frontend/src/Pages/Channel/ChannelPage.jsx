...
// const ChannelPage = () => {
//   const { channelId } = useParams();
//   const { allChannelData, userData, allVideosData } = useSelector(
//     (state) => state.user
//   );

//   const channel = allChannelData?.find((c) => c._id === channelId);
//   const [channelState, setChannelState] = useState(channel);
//   const [isSubscribed, setIsSubscribed] = useState(false);
//   const [loading, setLoading] = useState(false);
//   const [activeTab, setActiveTab] = useState("Videos");
...
//           setDurations((prev) => ({ ...prev, [video._id]: time }));
//         });
//       });
//     }
//   }, [allVideosData]);
...
//     if (!channelState || !userData) return;
//     setIsSubscribed(
//       channelState.subscribers?.some(
//         (sub) => (sub?._id ?? sub) === userData._id
//       )
//     );
//   }, [channelState, userData]);
...
//       setChannelState(res.data);
//       setIsSubscribed(
//         res.data.subscribers?.some((sub) => (sub?._id ?? sub) === userData._id)
//       );
//     } catch (err) {
//       console.error(err);
//     } finally {
...
//               {channelState?.subscribers?.length}
//             </span>{" "}
//             Subscribers •{" "}
//             <span className="font-semibold text-white">
//               {channelState?.videos?.length}
//             </span>{" "}
//             Videos
//           </p>
...
//         <button
//           className={`mt-2 sm:mt-0 text-xs sm:text-sm px-4 py-2 rounded-full ${
//             isSubscribed
//               ? "bg-black text-white border border-gray-700"
//               : "bg-white text-black"
//           }`}
...
//             key={tab}
//             onClick={() => setActiveTab(tab)}
//             className={`pb-2 sm:pb-3 relative font-medium transition ${
...
//         {activeTab === "CommunityPosts" && (
//           <div className="flex gap-5 flex-wrap">
//             {channelState.communityPosts?.map((p) => (
...
const ChannelPage = () => {
  ...
  useEffect(() => {
    // update channelState if store changes
  ...
  const handleSubscribe = async (channelId) => {
    ...
    try {
      ...
      setChannelState(res.data);
      setIsSubscribed(
        res.data.subscribers?.some((sub) => (sub?._id ?? sub)?.toString() === userData._id?.toString())
      );
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="text-white min-h-screen pt-4 sm:pt-6">
      ...
      <div className="relative flex flex-col sm:flex-row items-center gap-4 sm:gap-6 p-4 sm:p-6 rounded-xl bg-gradient-to-r from-gray-900 via-black to-gray-900 shadow-xl flex-wrap">
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          <p className="text-gray-400 mt-1 sm:mt-2 text-sm sm:text-base">
            <span className="font-semibold text-white">
              {channelState?.subscribers?.length ?? channelState?.subscribersCount ?? 0}
            </span>{" "}
            Subscribers •{" "}
            <span className="font-semibold text-white">
              {channelState?.videos?.length ?? 0}
            </span>{" "}
            Videos
          </p>
          <p className="text-gray-300 text-xs sm:text-sm mt-1 sm:mt-2 line-clamp-2">
        ...
        <button
          className={`mt-2 sm:mt-0 text-xs sm:text-sm px-4 py-2 rounded-full ${
            isSubscribed ? "bg-black text-white border border-gray-700" : "bg-white text-black"
          }`}
          onClick={() => handleSubscribe(channelState?._id)}
          disabled={loading}
        >
      ...
      <div className="px-4 sm:px-6 space-y-8">
        ...
        {activeTab === "Playlists" && (
          <div className="flex gap-5 flex-wrap">
            {channelState.playlists?.map((p) => (
              <PlaylistCard key={p._id} id={p._id} title={p.title} videos={p.videos} saveBy={p.saveBy} />
            ))}
          </div>
        )}
